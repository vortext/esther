...
** DONE simple sentiment analysis for energy
** DONE #keywords (hash tags instead of namespaced concepts?)
** DONE weird bug where login shows in memory (enforce redirect). Not fixed ...
- Steps to reproduce: clear cookies, send message.
** DONE use llama.clj instead of shell
- [X] make it work again with master llama.cpp
- [X] implement mirostat params
- [X] implement grammar
- [X] get rid of shell impl (generate better "prompt" to complete)
- [X] Try https://huggingface.co/PygmalionAI/mythalion-13b (not better?)
** DONE move handlebars in system, better Helper in llm.clj
...
- [ ] Disable text input while in modal confirm
